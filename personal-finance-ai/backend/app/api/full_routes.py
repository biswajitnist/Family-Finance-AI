...
from app.services.loan_balance_service import loan_read, matched_loan_transactions
...
def update_loan(
    ...
    loan = _update(db, Loan, item_id, payload)
    return loan_read(db, loan)
...
def list_loan_transactions(
    ...
    loan_count = len(db.scalars(select(Loan.id)).all())
    return [
        {
        ...
        for match in matched_loan_transactions(db, loan, loan_count)
    ]
...
def list_insurance(db: Session = Depends(get_db)) -> list[Insurance]:
    return _list(db, Insurance)
...
def create_insurance(
    ...
    return _create(db, Insurance, payload)
...
def update_insurance(
    ...
    return _update(db, Insurance, item_id, payload)
