...
async def store_document(
    ...
    if extension == ".pdf":
        try:
            ...
            processing_message = "PDF could not be inspected."
            processing_progress = 0
    document = Document(
        user_id=1,
        file_name=filename,
...
def process_document(
    ...
    try:
        ...
        if suffix == ".pdf":
            ...
            db.commit()
            result = extract_pdf(path, force_ocr=force_ocr, password=password)
            text = result.text
        ...
        if "password" in str(exc).casefold():
            document.status = "password_required"
            document.processing_stage = "locked"
            document.processing_message = (
                "This PDF is password protected. Enter the password to preview "
                "or process it."
            )
