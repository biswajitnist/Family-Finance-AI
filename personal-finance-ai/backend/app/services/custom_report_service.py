...
def _loan_balance_expression() -> ColumnElement[Any]:
    return (
        select(
            func.coalesce(
                func.sum(LoanLedgerEntry.signed_amount),
...
SOURCES: dict[str, SourceDefinition] = {
    ...
    "loans": SourceDefinition(
        ...
        fields={
            ...
            "direction": _field("Direction", "text", Loan.direction),
            "loan_type": _field("Loan Type", "text", Loan.loan_type),
            "current_balance": _field(
                "Outstanding Balance",
                "number",
                _loan_balance_expression(),
            ),
