...
class LoanCreate(MoneyModel):
    ...
    interest_mode: str = "manual"
    interest_calculation_basis: str = "daily_balance"
    interest_posting_frequency: str = "manual"
    payment_allocation_rule: str = "interest_first"
    status: str = "active"
...
class LoanSettingsUpdate(MoneyModel):
    interest_calculation_basis: str = "daily_balance"
    interest_posting_frequency: str = "manual"
    payment_allocation_rule: str = "interest_first"
    monthly_payment: Decimal = Decimal("0")
...
class LoanRead(LoanCreate):
    ...
    posted_interest_balance: Decimal = Decimal("0")
    payments_applied: Decimal = Decimal("0")
    matched_transaction_count: int = 0
    effective_interest_rate: Decimal = Decimal("0")
    monthly_interest_estimate: Decimal = Decimal("0")
    interest_rate_changes: list["LoanInterestRateRead"] = Field(default_factory=list)
    ledger_entries: list["LoanLedgerEntryRead"] = Field(default_factory=list)
    ledger_total: Decimal = Decimal("0")
    accumulated_interest_as_of_today: Decimal = Decimal("0")
    total_borrowed: Decimal = Decimal("0")
    total_repaid: Decimal = Decimal("0")
    total_interest_posted: Decimal = Decimal("0")
    total_payable_today: Decimal = Decimal("0")
    documents: list[DocumentRead] = Field(default_factory=list)


class LoanInterestRateCreate(MoneyModel):
    effective_date: date
    interest_rate: Decimal
    notes: str | None = None


class LoanInterestRateRead(LoanInterestRateCreate):
    id: int
    loan_id: int
    created_at: datetime


class LoanLedgerEntryCreate(MoneyModel):
    entry_date: date
    description: str
    entry_type: str = "loan_taken"
    direction: str = "debit"
    amount: Decimal
    principal_component: Decimal = Decimal("0")
    interest_component: Decimal = Decimal("0")
    currency: str = "EUR"
...
class LoanLedgerEntryRead(LoanLedgerEntryCreate):
    id: int
    loan_id: int
...
class LoanInterestCalculationRequest(MoneyModel):
    period_start: date
    period_end: date
    post: bool = False


class LoanInterestCalculationRead(MoneyModel):
    id: int | None = None
    loan_id: int
    period_start: date
    period_end: date
    ...
    currency: str
    status: str = "preview"
    posted_ledger_entry_id: int | None = None
    segments: list[dict] = Field(default_factory=list)


class LoanYearSummary(MoneyModel):
    year: int
    opening_balance: Decimal
    loan_taken: Decimal
    interest_posted: Decimal
    repayments: Decimal
