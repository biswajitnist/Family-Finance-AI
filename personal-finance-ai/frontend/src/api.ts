...
export const api = {
  ...
  transactions: (validated?: boolean, documentId?: number) =>
    request<Transaction[]>(
      `/transactions/base?${new URLSearchParams({
  ...
  documentPreviewUrl: (id: number, page = 1, password?: string) =>
  ...
  loanInterestRates: (loanId: number) =>
    request<ResourceRecord[]>(`/loans/${loanId}/interest-rates`),
  createLoanInterestRate: (loanId: number, body: Record<string, unknown>) =>
    request<ResourceRecord>(
      `/loans/${loanId}/interest-rates`,
  ...
  updateLoanInterestRate: (
    ...
    request<ResourceRecord>(
      `/loans/${loanId}/interest-rates/${rateId}`,
      jsonOptions("PUT", body),
    ),
  deleteLoanInterestRate: (loanId: number, rateId: number) =>
    request<void>(`/loans/${loanId}/interest-rates/${rateId}`, {
      method: "DELETE",
    }),
  loanInterestAsOf: (loanId: number, asOf: string) =>
    request<ResourceRecord>(
      `/loans/${loanId}/interest-as-of?as_of=${encodeURIComponent(asOf)}`,
    ),
  loanLedgerEntries: (loanId: number) =>
    request<ResourceRecord[]>(`/loans/${loanId}/ledger-entries`),
  createLoanLedgerEntry: (loanId: number, body: Record<string, unknown>) =>
    request<ResourceRecord>(
      `/loans/${loanId}/ledger-entries`,
      jsonOptions("POST", body),
    ),
  deleteLoanLedgerEntry: (loanId: number, entryId: number) =>
    request<void>(`/loans/${loanId}/ledger-entries/${entryId}`, {
      method: "DELETE",
    }),
  updateLoanSettings: (loanId: number, body: Record<string, unknown>) =>
    request<ResourceRecord>(
      `/loans/${loanId}/settings`,
      jsonOptions("PUT", body),
    ),
  uploadLoanDocument: (loanId: number, file: File, notes?: string) => {
    const form = new FormData();
    form.append("file", file);
    if (notes) form.append("notes", notes);
    return request<ResourceRecord>(`/loans/${loanId}/documents/upload`, {
      method: "POST",
      body: form,
    });
  },
  unlinkLoanDocument: (loanId: number, documentId: number) =>
    request<ResourceRecord>(`/loans/${loanId}/documents/${documentId}`, {
      method: "DELETE",
    }),
  uploadInsuranceDocument: (policyId: number, file: File, notes?: string) => {
    const form = new FormData();
    form.append("file", file);
    if (notes) form.append("notes", notes);
    return request<ResourceRecord>(`/insurance/${policyId}/documents/upload`, {
      method: "POST",
      body: form,
    });
  },
  unlinkInsuranceDocument: (policyId: number, documentId: number) =>
    request<ResourceRecord>(`/insurance/${policyId}/documents/${documentId}`, {
      method: "DELETE",
    }),
