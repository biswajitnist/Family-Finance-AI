  ...
  async function process(id: number) {
    ...
    const password =
      document?.status === "password_required"
        ? window.prompt("This PDF is password protected. Enter the password:")
        : undefined;
    if (document?.status === "password_required" && !password) {
      setMessage("Password is required to process this PDF.");
      return;
    }
...
export function DocumentViewer({
  ...
  const previewUrl = api.documentPreviewUrl(
    document.id,
    page,
  ...
  const needsPassword = document.status === "password_required" || passwordMode;
  ...
  useEffect(() => {
    ...
    setPreviewPassword("");
    setPreviewError("");
    setPasswordMode(document.status === "password_required");
    ...
    >
      <article className="document-viewer panel" role="dialog" aria-modal="true">
        ...
        <div className="document-viewer-body">
          {isImage ? (
          ...
          ) : isPdf && needsPassword && !previewPassword ? (
            <form
              className="document-password-panel"
              onSubmit={(event) => {
                event.preventDefault();
                if (!password.trim()) {
                  setPreviewError("Enter the PDF password to preview it.");
                  return;
                }
                setPreviewError("");
            ...
            >
              ...
              <span>
                Enter the password to preview this document here. The password is
                only used for this preview and is not saved.
              </span>
              <input
                autoFocus
              ...
              {previewError && <small className="danger-text">{previewError}</small>}
              <a
                className="secondary-button"
                download
                href={originalUrl}
            ...
            <img
              alt={`${document.file_name} preview page ${page}`}
              key={previewUrl}
              onError={() =>
                {
                  if (isPdf && !previewPassword) {
                    setPasswordMode(true);
                    setPreviewError(
                      "Preview could not be loaded. If this PDF is password protected, enter the password.",
                    );
                    return;
                  }
                  setPreviewError(
