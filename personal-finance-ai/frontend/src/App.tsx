...
> = {
  ...
  loans: {
    path: "loans",
    title: "Loans",
    description: "Monitor balances, interest rates, monthly payments, and payoff dates.",
    fields: [
      { name: "lender", label: "Lender", required: true },
      { name: "loan_type", label: "Loan type", defaultValue: "personal" },
      {
...
export default function App() {
  ...
  const insuranceCoverage = insurancePolicies.reduce(
    (sum, item) => sum + numberField(item, "coverage_amount"),
    0,
  );
  const monthlyInsurancePremium = insurancePolicies.reduce(
